...
class PhersistentMySQL {
  ...
  private function save_instance_recursive($phi)
  {
    ...
    foreach ($hones as $attr => $value)
    {
      ...
      else if ($value->get_id() == null) // insert, is always dirty if not saved
      {
      ...
      else
      {
        ...
        // TEST: this _id should already be set when we set a HO that has ID
        $phi->set($attr .'_id', $idho); // FK set on owner
      }
    }
  ...
  private function update_instance_recursive($phi)
  {
    ...
    foreach ($hones as $attr=>$value)
    {
      ...
      //
      // NOTE: set_xxx_id(null)  shouldn't have any effect, since this is an internal field,
      //       it should be managed by the library not by the user. So the only way of
      //       setting NULL to a has one is to set_xxx(null)
      //
  ...
  private function table_to_insert($table)
  {
    ...
    foreach ($table['columns'] as $col => $val)
    {
      ...
      if (BasicString::endsWith($col, '_id') && $val == null)
      {
        continue;
      }
  ...
  public function get_table_name_ph(Phersistent $ph)
  {
    ...
    while ($ph->get_parent() != 'CaboLabs\Phersistence\phersistent\Phersistent')
    {
  ...
  public function class_to_table_name($class_name)
  {
    ...
    // TheClass => the_class
    return BasicString::camel_to_snake($simple_name);
  }
